...
import { CompletedItemsCount, Divider, FormattedItemText, Modal } from '@/shared/components'

import { useAuth, useList } from '@/hooks'
import { createList as createListOnDB, updateUserLists } from '@services/list'

import { DEFAULT_ICON_PROPS, LIST_ROUTE, LIST_SETTINGS_ROUTE, QUOTES } from '@/constants'
...
export default function Lists() {
  ...
  const { lists, saveSelectedList, deleteList, updateRecentlyAccessedLists } = useList()
  ...
  const handleClickOnListName = (list: ListProps) => {
    ...
    updateRecentlyAccessedLists(list)
  ...
  return (
    <>
      ...
      <div className="mb-2 flex w-full items-center justify-between px-4">
        <h1 className="default-header-title">Listas</h1>
              ...
              <div
                className={classNames(
                  'flex items-center py-2 pl-4 pr-2 transition-all hover:bg-lightGray dark:hover:bg-darkInputBackground',
                  {
                    ...
                    'rounded-b-default': index === lists.length - 1,
                  }
                )}
              >
                {/* list text */}
